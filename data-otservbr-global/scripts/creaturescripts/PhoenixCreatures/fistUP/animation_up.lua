local AnimationUp = CreatureEvent("AnimationUp")

function AnimationUp.onAdvance(player, skill, oldLevel, newLevel)

	if skill ~= SKILL_FIST or newLevel <= oldLevel then
		return true
	end
	...
	if newLevel > oldLevel then
		...
		player:say("[AttackSpeed UP]", TALKTYPE_MONSTER_SAY)
