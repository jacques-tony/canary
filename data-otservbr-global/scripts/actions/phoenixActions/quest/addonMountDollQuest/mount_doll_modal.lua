local mounts = {
    ...
    -- Config
        storageID = 8191, -- Storage value.
        action_aid = 33225, -- ActionID.
...
local mountsModal = Action()
function mountsModal.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    player:sendMountWindow(mounts)
end
mountsModal:aid(mounts.action_aid)
mountsModal:register()
