local config = {
	[20272] = {
		targetId = 20302, -- Target ID.
		bossName = "Zavarash", -- boss name
		keyPlayerPosition = Position(17564, 17561, 5), -- Where the player should be.
		newPosition = Position(18235, 17464, 12), -- Position to teleport
		bossPosition = Position(18233, 17460, 12), -- Boss Position
		centerPosition = Position(18235, 17464, 12), -- Center Room
		exitPosition = Position(17564, 17563, 5), -- Exit Position
		rangeX = 20, -- Range in X
		rangeY = 20, -- Range in Y
		time = 15, -- time in minutes to remove the player
	},
	[20271] = {
		targetId = 20300, -- Target ID.
		bossName = "Horadron", -- boss name
		keyPlayerPosition = Position(17566, 17561, 5), -- Where the player should be.
		newPosition = Position(18275, 17463, 12), -- Position to teleport
		bossPosition = Position(18274, 17459, 12), -- Boss Position
		centerPosition = Position(18275, 17463, 12), -- Center Room
		exitPosition = Position(17566, 17563, 5), -- Exit Position
		rangeX = 20,
		rangeY = 20,
		time = 15, -- time in minutes to remove the player
	},
	[20270] = {
		targetId = 20304, -- Target ID.
		bossName = "Terofar", -- boss name
		keyPlayerPosition = Position(17562, 17561, 5), -- Where the player should be.
		newPosition = Position(18194, 17463, 12), -- Position to teleport
		bossPosition = Position(18192, 17460, 12), -- Boss Position
		centerPosition = Position(18194, 17463, 12), -- Center Room
		exitPosition = Position(17562, 17563, 5), -- Exit Position
		rangeX = 20,
		rangeY = 20,
		time = 15, -- time in minutes to remove the player
	},
}

local keys = Action()

function keys.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local tmpConfig = config[item.itemid]
	if not tmpConfig then
		return true
	end

	if target.itemid ~= tmpConfig.targetId then
		return true
	end

	local creature = Tile(tmpConfig.keyPlayerPosition):getTopCreature()
	if not creature or not creature:isPlayer() then
		return true
	end

	if roomIsOccupied(tmpConfig.centerPosition, false, tmpConfig.rangeX, tmpConfig.rangeY) then
		player:sendCancelMessage("There is someone in the room.")
		return true
	end

	local monster = Game.createMonster(tmpConfig.bossName, tmpConfig.bossPosition)
	if not monster then
		return true
	end

	-- Send message
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have entered an ancient demon prison cell!")
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have fifteen minutes to kill and loot this boss, else you will lose that chance.")

	-- Let's roll
	addEvent(clearBossRoom, 60 * tmpConfig.time * 1000, player:getId(), tmpConfig.centerPosition, false, tmpConfig.rangeX, tmpConfig.rangeY, tmpConfig.exitPosition)
	item:remove()
	player:teleportTo(tmpConfig.newPosition)
	player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
	return true
end

keys:id(20270, 20271, 20272)
keys:register()
