local monsters = {
	[1] = {name = "Scarlett", storage = Storage.GraveDanger.CobraBastion.Questline},
	[2] = {name = "Horestis", storage = 376357},
}
	...
	if modalWindowId == 1000 then
		if buttonId == 100 then 
			if not player:removeItem(onUse_item, 1) then 
				player:sendTextMessage(MESSAGE_INFO_DESCR, "O item precisa est� no seu personagem para resetar o boss.")
				return true
			end
			...
			if bosses_reseted == true then
				player:sendTextMessage(MESSAGE_INFO_DESCR, "Voc� acabou de resetar todos os bosses com sucesso!")
			else
...
function Player.getListBosses(self)
	...
	local message = "Ao clicar em resetar, todos os bosses dessa lista ser� resetado.."
...
function action.onUse(player, item, fromPosition, itemEx, toPosition, isHotkey)
	...
	if player:getListBosses() == 0 then
		player:sendTextMessage(MESSAGE_INFO_DESCR, "A lista est� v�zia.")
	end
    return true
end
