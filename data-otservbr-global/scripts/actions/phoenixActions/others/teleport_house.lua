...
function teleportHouse.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    -- Verificar se o jogador está PZ Locked
    if player:isPzLocked() then
        player:sendCancelMessage("Não é possível usar enquanto você estiver em combate.")
        return true
    end

    -- Verificar se o jogador NÃO está em uma zona de proteção ao jogador (PZ zone)
    local pzInfo = getTilePzInfo(player:getPosition())
    if not pzInfo then
        player:sendCancelMessage("Não é possível usar fora de uma zona de proteção ao jogador.")
        return true
    end
