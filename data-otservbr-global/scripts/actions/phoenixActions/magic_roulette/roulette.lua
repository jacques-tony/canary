local config = {
    ...
    prizePool = {
        {itemId = 32584, count = {1, 1},    chance = 500}, -- {itemId = itemid, count = {min, max}, chance = chance/10000} (crystal coins)
        ...
        {itemId = 4010, count = {1, 1},    chance = 500},
        {itemId = 43732, count = {1, 1},    chance = 500},
        {itemId = 43734, count = {1, 1},    chance = 500}
    
    },
    roulettePositions = { -- hard-coded to 7 positions.
        Position(17573, 17551, 6),	
        Position(17574, 17551, 6),
        Position(17575, 17551, 6),
        Position(17576, 17551, 6),
        Position(17577, 17551, 6),
        Position(17578, 17551, 6), -- position 4 in this list is hard-coded to be the reward location, which is the item given to the player
        Position(17579, 17551, 6),
        Position(17580, 17551, 6),
        Position(17581, 17551, 6),
        Position(17582, 17551, 6),
        Position(17583, 17551, 6),		 
    }
}
...
local function rewardPlayer(playerId, leverPosition)
    ...
    if chancedItems[#chancedItems - 3] <= config.rouletteOptions.rareItemChance_broadcastThreshold then
        Game.broadcastMessage("O Jogador " .. player:getName() .. " Ganhou uma " .. item:getName() .. " na Roleta da Sorte!", MESSAGE_GAME_HIGHLIGHT)
    end
end
...
local RouletteOne = Action()

function RouletteOne.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getId() == config.lever.right then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, "Roleta em acao. Espere um pouco.")
        return true
    end
    ...
    if player:getItemCount(config.playItem.itemId) < config.playItem.count then
        if player:getStorageValue(config.rouletteOptions.rouletteStorage) < 1 then
            player:sendTextMessage(MESSAGE_STATUS_SMALL, "Voce precisa de um " .. config.playItem.count .. " " .. (ItemType(config.playItem.itemId):getName()) .. " para usar a Roleta.")
            return true
        end
...
RouletteOne:aid(config.actionId)
RouletteOne:register()
