...
function scroll.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if player:getLevel() < 51 then
        ...
        return true
    elseif playerResets(player) < 1 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Only a hero with 1 reset or more can decipher this scroll.")
        return true
    end

    local scrollData = promotionScrolls[item:getId()]
    if player:getStorageValue(scrollData.storageName) == 1 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have already deciphered this scroll.")
        return true
    end

    player:setStorageValue(scrollData.storageName, 1)
